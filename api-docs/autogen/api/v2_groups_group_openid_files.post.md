...
上传图片/视频/语音到群聊，返回 file\_info 用于发送消息接口的 media 字段。 srv\_send\_msg=true 时直接发送消息并占用主动消息频次；false 时仅返回 file\_info。 用群接口上传的文件仅能发送到群聊。 文件类型与大小限制:
...
← [单聊分片上传完成](/wiki/develop/api-v2/autogen/api/v2_users_user_id_upload_part_finish.post.html) [群聊富媒体预上传](/wiki/develop/api-v2/autogen/api/v2_groups_group_id_upload_prepare.post.html) →
