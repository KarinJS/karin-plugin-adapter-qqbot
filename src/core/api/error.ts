...
export const httpStatusMap: Record<number, string> = {
  ...
  201: '异步操作成功，虽然说成功，但是会返回一个 error body，需要特殊处理',
  202: '异步操作成功，虽然说成功，但是会返回一个 error body，需要特殊处理',
  401: '认证失败',
  404: '未找到 API',
  405: 'http method 不允许',
  429: '频率限制',
...
export const errorCodeMap: Record<number, string> = {
  ...
  304019: 'AT_EVERYONE_TIMES @全体成员次数超限',
...
const rangeDefs: RangeDef[] = [
  { min: 301000, max: 301099, message: '子频道权限错误' },
  { min: 501000, max: 501999, message: '公告错误' },
  { min: 502000, max: 502099, message: '禁言相关错误' },
  { min: 504000, max: 504999, message: '消息频率相关错误' },
  { min: 610000, max: 619999, message: '频道权限错误' },
  { min: 620001, max: 629999, message: '表情表态错误' },
  { min: 630001, max: 639999, message: '互动回调数据更新错误' },
  { min: 1000000, max: 2999999, message: '发消息错误' },
  { min: 3000000, max: 3999999, message: '编辑消息错误' },
]

/**
 * 根据 code 查找对应的中文错误描述
 * @param code - 业务错误码
 * @returns 中文错误描述，未找到则返回 undefined
 */
export const getErrorMessage = (code: number): string | undefined => {
  // 精确匹配
  const exact = errorCodeMap[code]
  if (exact) return exact

  // 范围匹配
  for (const range of rangeDefs) {
    if (code >= range.min && code <= range.max) {
      return `${range.message} (code: ${code})`
    }
  }
...
/**
 * 格式化 OpenAPI 错误为可读字符串
 * @param status - HTTP 状态码
 * @param code - 业务错误码
 * @param message - 原始消息
 * @returns 格式化后的错误字符串
 */
export const formatOpenAPIError = (
  status: number,
  code?: number,
  message?: string
): string => {
  ...
  const statusText = httpStatusMap[status]
  if (statusText) {
  ...
  if (code !== undefined) {
    const codeText = getErrorMessage(code)
    if (codeText) {
      parts.push(`[Code ${code}] ${codeText}`)
    } else {
